...
```{r}
#sd <- read.csv(file="http://faculty.seattlecolleges.edu/faculty/brian.holt/psych217/data/Student_Height_Weight_EngGpa_1-16-2008.csv",header=T)[-c(1)] 

#write_csv(sd,file='height_gpa.csv')
sd <- read.csv(file='height_gpa.csv')
...
ggplot(data=data.frame(x=1:10,y=10:1),aes(x,y))+
  geom_line()+
...
sex.d %>% select(id,Age,sex) %>%  distinct() %>% 
  summarise(mean=mean(Age)) %>% kbl() %>% kable_styling(position = 'float_left')
```
# Median Example
...
sex.d %>% select(id,Age) %>%  distinct() %>% 
  ...
  ggplot(aes(x=position,y=0))+geom_line(size=.1)+
  ...
  ylim(-.1,.1)+
  geom_vline(xintercept = ifelse(sex.age.median$n[1]%%2==0,sex.age.median$n[1]/2,(sex.age.median$n[1]+1)/2),color='blue')+
  ...
  # 
  # geom_text(aes(label=Age),hjust=.5, vjust=2) +
  # theme_void()+
  # theme(axis.line.y=element_blank(),
  #         axis.text.y=element_blank(),
  #         axis.ticks.y=element_blank(),
...
# Mode Example
```{r}
sex.d %>% select(id,Age) %>%  distinct() %>% group_by(Age) %>% 
