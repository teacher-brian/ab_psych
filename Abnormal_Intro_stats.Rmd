...
```{r}
 sex.d %>% select(id,Age,sex) %>%  distinct() %>% 
  filter(Age<49) %>% 
   ggplot(aes(x=Age))+geom_histogram(bins=15)+
...
It's not quite correct, but if you add up all the ages on the left side, you get about `r sum(Age[Age<mean_age[[1]]])`, and `r sum(Age[Age>mean_age[[1]]] )` on the left side.
...
sd %>% 
  ggplot(aes(x=Height))+geom_histogram(bins=15)+
