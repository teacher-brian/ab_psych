...
sex.d %>% select(id,Age,sex) %>%  distinct() %>% 
  summarise(mean=mean(Age)) %>% kbl() %>% kable_styling()
```
# Median Example

```{r,fig.dim=c(8,2)}
sex.age.median <- sex.d %>% select(id,Age) %>%  distinct() %>% 
    summarise(q=quantile(Age,probs = .5))

sex.d %>% select(id,Age) %>%  distinct() %>% 
   mutate(Age=jitter(Age, amount = 0.1)) %>% 
  ggplot(aes(x=Age,y=0))+geom_line()+
  geom_text(aes(label=round(Age,0)),vjust=1)+
  geom_vline(xintercept = sex.age.median$q)+
