...
You could say it looks a bit like this:
...
sex.d %>% select(id,Age,sex) %>%  distinct() %>% 
  summarise(mean=mean(Age)) %>% kbl() %>% kable_styling(full_width = F)
...
sex.d %>% select(id,Age) %>%  distinct() %>% 
  ...
  ylim(-.05,.08)+
  geom_segment(aes(x = sex.age.median$q[3],y=0,xend=sex.age.median$q[3],yend=.05),
               color='blue')+
